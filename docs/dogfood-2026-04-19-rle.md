...
voided the documented safety guarantee of `ghci_refactor`
...
|---|-----------------------|-------------|-----------------------------------------------|------------|
| F-01 | `ghci_deps add`    | 🐛 **High**     | Produces unparseable `.cabal`, returns `success=true` | `5adc350` |
| F-02 | `ghci_deps add`    | 🟡 Low      | Indent style inconsistent with existing block | `5adc350` |
| F-03 | `ghci_deps`        | 🐛 **High**     | No way to target `test-suite` / `benchmark` stanza | `5adc350` |
| F-04 | `ghci_arbitrary`   | 🐛 **High**     | Bails on records with `!` strict fields + GHC 9.x kind header | `42d830c` |
| F-05 | `ghci_suggest`     | 🐛 **High**     | Emits false list laws for `[a] -> [Run a]` (inner-type check missing) | `42d830c` |
| F-06 | `ghci_quickcheck`  | 🐛 **High**     | Session spawned without QuickCheck; `quickCheck` not in scope | `6bc3354` |
| F-07 | `ghci_refactor`    | 🟡 Medium   | Scoped rename leaves out-of-range definition intact (self-healing once F-08 lands) | `bc5b3ce` |
| F-08 | `ghci_refactor`    | 🚨 **Critical** | `:unset -fdefer-*` isn't the inverse of `:set -f-defer-*`; deferred flags leak across loads; compile-verify silently passes broken renames | `bc5b3ce` |
| F-09 | `ghci_coverage`    | 🐛 **High**     | Modern cabal writes HPC only to HTML; parser saw empty stdout | `fa92d09` |

## Wins preserved (anti-regression markers)

| # | Marker                                                                       |
|---|------------------------------------------------------------------------------|
| W-01 | `ghci_create_project(name="dogfood-rle")` produced 4 files at first shot  |
| W-02 | Scaffold from `ghci_create_project` compiles clean on first `ghci_load`   |
| W-03 | `ghci_load(diagnostics=true)` against the RLE module returned 0 errors + 0 warnings |
...
| W-08 | `ghci_batch(fail_fast=false)` correctly reported `ok=5 failed=1` for a mixed run |
| W-09 | Post-edit invariant check on `ghci_deps` (shipped with F-01 fix) would now reject the original corrupted-cabal output |

## Timeline

1. **T+0** — scaffolded the project via `ghci_create_project`. W-01, W-02.
2. **T+5m** — asked `ghci_deps add QuickCheck` → broke the .cabal silently. F-01, F-02. Immediately discovered F-03 as the reason I needed workaround indirection (QuickCheck should live in test-suite, not library).
...
5. **T+30m** — called `ghci_arbitrary("Run")` → F-04 (two bugs sharing a root). Fixed `parseConstructors` + `groupTokens` to handle the GHC 9.x kind header and record braces as grouping. Commit `42d830c`.
6. **T+40m** — called `ghci_suggest("encode")` → F-05. Fixed the list rules to enforce `argInner == retInner`. Same commit `42d830c`.
7. **T+45m** — called `ghci_quickcheck` against the persistent session → F-06 (`Variable not in scope: quickCheck`). Added `--build-depends QuickCheck` to every session spawn. Commit `6bc3354`.
8. **T+55m** — exercised `ghci_refactor(rename_local)` → F-07 (narrow-scope leaves the definition) and F-08 (the critical one — deferred flags leaked, compile-check lied). Fixed `loadModuleWith` to use `-fno-defer-*` + eagerly clear on `Strict` entry. Commit `bc5b3ce`.
9. **T+70m** — `ghci_coverage` → cabal test passed all 4 properties (W-04..W-07) but metrics list was empty. F-09. Wired `hpc report` post-pass. Commit `fa92d09`.
...
### F-01 / F-02 — `ghci_deps add` indent
...
`ghci_create_project`), that yielded the field's own column. Cabal
...
### F-03 — No stanza targeting in `ghci_deps`
...
### F-04 — `ghci_arbitrary` parser gives up on normal records
...
### F-06 — `ghci_quickcheck` with no QuickCheck
...
invocation of the `ghci_quickcheck` tool hit
...
### F-07 / F-08 — `ghci_refactor` compile-check lied
...
flags indefinitely after the first `ghci_load(diagnostics=true)`
call — all subsequent compile-checks silently deferred their
errors.

This voided the `ghci_refactor` safety guarantee: broken edits
...
### F-09 — `ghci_coverage` reported no metrics
...
- `ghci_regression(run)` — property persistence was blocked by F-06.
- `ghci_goto` — never called.
- `ghci_doc` — never called.
- `ghci_complete` — never called.
- `ghci_hls hover` — HLS not on PATH.
- `ghci_format` — fourmolu / ormolu not on PATH.
- `ghci_refactor(extract_binding)` — happy path only.
...
```
fa92d09 fix(mcp-server-haskell): ghci_coverage parses hpc report text, not cabal stdout (F-09)
bc5b3ce fix(mcp-server-haskell): CRITICAL — Deferred GHC flags leaked across loads, voiding ghci_refactor safety (F-08)
...
5adc350 fix(ghci_deps): correct continuation indent, add stanza targeting, post-edit invariant check
```
...
| `ghci_coverage` recognises modern cabal layout                          | ✅ (F-09) |
